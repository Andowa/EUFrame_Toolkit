<?php
namespace library\EssentialUnifiedDebug;
...
class EUDebug{
    public static function Debug($mode='0'){
        ...
        if(isset($debug)){
            EUDebug::WriteDebug($mode,$debug);
        }
    }
    public static function WriteDebug($mode,$debug){
        $type=$debug["type"];
        $message=$debug["message"];
        $file=str_replace(EUF_ROOT,"",$debug["file"]);
        $line=$debug["line"];
    ...
    public static function Error($type='',$path=''){
        ...
        echo$path."<br/>";
        echo"<span style='font-size:11px;'>This is an error report on EU Framework.</span>";
        echo"</p>";
        exit();
    }
}