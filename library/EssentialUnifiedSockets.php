<?php
namespace library\EssentialUnifiedSockets;
/**
       * --------------------------------------------------------       
       *  |         _____     _   _                           |           
       *  |        | ____|   | | | |                          |            
...
class EUSockets{
    ...
    private function DealMsg($socket, $recv_msg) {
        ...
        switch ($msg_type) {
            ...
            //单对多消息
            case 'eu-room':
                $response['type'] = 'eu-room';
                $response['roomid'] = $recv_msg['roomid'];
                $response['from'] = $this->sockets[$socket]['uname'];
                $response['content'] = $msg_content;
                $response['sendtime'] = date('Y-m-d H:i:s',time());
                if(!empty($recv_msg['item']) && !empty($recv_msg['roomid'])){
                    file_put_contents(APP_ROOT.'/'.$recv_msg['item'].'/room-'.$recv_msg['roomid'].'-'.date("Ymd").'.eulog', json_encode($response) . "\r\n", FILE_APPEND);
                }
                break;
            //单对单消息
            case 'eu-chat':
                $response['type'] = 'eu-chat';
                ...
                if(!empty($recv_msg['item'])){
                    ...
                    file_put_contents(APP_ROOT.'/'.$recv_msg['item'].'/chat-'.$recv_msg['startuid'].'-'.$recv_msg['enduid'].'.eulog', json_encode($response) . "\r\n", FILE_APPEND);
                    }else{
                    file_put_contents(APP_ROOT.'/'.$recv_msg['item'].'/chat-'.$recv_msg['enduid'].'-'.$recv_msg['startuid'].'.eulog', json_encode($response) . "\r\n", FILE_APPEND);
                    }
                }
                break;
        }
