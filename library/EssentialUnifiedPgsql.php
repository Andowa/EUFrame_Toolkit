<?php
namespace library\EssentialUnifiedPgsql;
...
class EUPgsql{   
    ...
    public static function QueryData($table,$field='',$where='',$order='',$limit='',$lang='0'){
        ...
        $querynum=empty($limit) ? $curnum : EUPgsql::QueryNum("select ".$field." from ".$table." ".$where." ".$order);
        while($rows=@pg_fetch_object($result)){
            $array[] = EUPgsql::ObjectToArray($rows);
        }
    ...
    public static function ObjectToArray($obj){
        ...
        foreach($obj as $key => $value){
            if(is_array($value) || is_object($value)){
                $ret[$key] = EUPgsql::ObjectToArray($value);
            }else{
                $ret[$key] = $value;
            }
        }
