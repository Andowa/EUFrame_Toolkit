<?php
namespace library\EssentialUnifiedMysql;
...
class EUMysql{
    ...
    public static function GetConfig(){
        return EssentialUnifiedInc\EUInc::GetConfig();
    }
    /**
     * 连接Mysqli
     */    
    public static function GetMysql(){
        $config=EUMysql::GetConfig();
    ...
    public static function QueryData($table,$field='',$where='',$order='',$limit='',$lang='0'){
        ...
        if(EUMysql::ModTable($table)):
            $sql="select ".$field." from `".$table."` ".$where." ".$order." ".$limit;
            $query=$db->query($sql);
            $curnum=mysqli_num_rows($query);
            $querynum=empty($limit) ? $curnum : EUMysql::QueryNum("select ".$field." from ".$table." ".$where." ".$order);
            $querydata=array(); 
            $xu=0;
    ...
    public static function SearchData($keyword){
		...
		if(!empty($keyword)):
			...
			if(mysqli_num_rows($sdata)>0):
			    EUMysql::UpdateData("cms_search",array("hit"=>"hit+1"),"keyword ='$keyword' and lang='$language'");
			endif;
		endif;
		$data=array();
        ...
        if(!empty($keyword) && $searchnum>0 && mysqli_num_rows($sdata)<=0):
			EUMysql::InsertData("cms_search",array("lang"=>$language,"keyword"=>$keyword));
        endif;
        $searchdata=array(); 
        $xu=0;
