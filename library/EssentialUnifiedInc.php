<?php
namespace library\EssentialUnifiedInc;
/**
       * --------------------------------------------------------       
       *  |         _____     _   _                           |           
       *  |        | ____|   | | | |                          |            
...
class EUInc{
    ...
    public static function GetConfig(){
        $string = file_get_contents(EUF_ROOT."/.eu.config");
    ...
    /**
     * 删除模块集合中的某个模块
     * @param array $arr EU模块集合，modules/module.config
     * @param string $key 模块标识名称
     * @return array 返回新的EU模块集合
     */
    ...
    public static function GetMod($type=0){
        ...
        if(empty($type) || $type==0){
            ...
            foreach($temp as $v){
                ...
                if(is_file($file."/essentialunified.config")){
                    $mods=file_get_contents($file."/essentialunified.config");
                    $modtype=EUInc::StrSubstr("<modtype>","</modtype>",$mods);
                    if($modtype==2){
                        $mid=EUInc::StrSubstr("<id>","</id>",$mods);
                        $catid=EUInc::StrSubstr("<itemid>","</itemid>",$mods);
                        $title=EUInc::StrSubstr("<modname>","</modname>",$mods);
                        $auther=EUInc::StrSubstr("<auther>","</auther>",$mods);
                        $url=EUInc::StrSubstr("<modurl>","</modurl>",$mods);
                        $mod[]=array("mid"=>$mid,"catid"=>$catid,"title"=>$title,"auther"=>$auther,"url"=>$url);
                    }
                }
            }
        }elseif($type==1){
            $modules=EUInc::Auth($config["EUCODE"],$config["EUFURL"],"module");
            ...
            $modules=EUInc::Auth($config["EUCODE"],$config["EUFURL"],"moduleorder");
            preg_match_all("/\<module\>(.*?)\<\/module\>/s",$modules,$moduleblocks);
            foreach($moduleblocks[1] as $module){
    ...
    public static function GetPlugin($type=0){
        ...
        if(empty($type) || $type==0){
            ...
            foreach($temp as $v){
                ...
                if(is_file($file."/essentialunified.config")){
                    $plugins=file_get_contents($file."/essentialunified.config");
                    $plugintype=EUInc::StrSubstr("<plugintype>","</plugintype>",$plugins);
                    if($plugintype==2){
                        $pid=EUInc::StrSubstr("<id>","</id>",$plugins);
                        $title=EUInc::StrSubstr("<pluginname>","</pluginname>",$plugins);
            ...
            $hooks=EUInc::Auth($config["EUCODE"],$config["EUFURL"],"plugin");
            ...
            $hooks=EUInc::Auth($config["EUCODE"],$config["EUFURL"],"pluginorder");
            preg_match_all("/\<hook\>(.*?)\<\/hook\>/s",$hooks,$hookblocks);
            foreach($hookblocks[1] as $hook){
    ...
    public static function GetTemplate($type=0){
        ...
        if(empty($type) || $type==0){
            ...
            foreach($temp as $v){
                ...
                if(is_file($file."/essentialunified.config")){
                    $temps=file_get_contents($file."/essentialunified.config");
                    $temptype=EUInc::StrSubstr("<temptype>","</temptype>",$temps);
                    if($temptype==2){
                        $tid=EUInc::StrSubstr("<id>","</id>",$temps);
                        $title=EUInc::StrSubstr("<title>","</title>",$temps);
                        $auther=EUInc::StrSubstr("<auther>","</auther>",$temps);
                        $description=EUInc::StrSubstr("<description>","</description>",$temps);
                        $template[]=array("tid"=>$tid,"title"=>$title,"auther"=>$auther,"description"=>$description);
                    }
                }
            }
        }elseif($type==1){
            $temps=EUInc::Auth($config["EUCODE"],$config["EUFURL"],"temp");
            ...
            $temps=EUInc::Auth($config["EUCODE"],$config["EUFURL"],"temporder");
            preg_match_all("/\<temp\>(.*?)\<\/temp\>/s",$temps,$tempblocks);
            foreach($tempblocks[1] as $temp){
    ...
    public static function InstallDev(){
        if(is_dir('install-dev')){
            if(file_exists(EUF_ROOT."/install-dev/essentialunified.lock")){
                return true;
            }else{
                return false;
            }
    ...
    public static function Plugin($pluginname,$pluginroot='index.php'){
        ...
        if(is_dir(APP_ROOT."/plugins/".$pluginname."")):
            if(empty($pluginroot)||EUInc::Contain(".php",$pluginroot)):
    ...
    public static function Auth($authcode,$authurl,$apitype){
        $fromurl=EUInc::CurPageUrl();
        $url="".$authurl."?AuthCode=".$authcode."&FromUrl=".$fromurl."&Type=".$apitype."";
        $content=EUInc::HttpGet($url);
        return str_replace("#","$",str_replace("&","=",EUInc::StrSubstr("<php>","</php>",$content)));
    }
    /**
     * 生成随机字符串
    ...
    public static function DetectEncoding($file) {
        $list = array('GBK', 'UTF-8', 'EUF-16LE', 'EUF-16BE', 'ISO-8859-1');
    ...
    public static function Download($file){
        ...
        if(!file_exists($file)){
            ...
            Header("Content-Disposition: attachment; filename=".EUInc::GetRandomString(8).".".substr(strrchr($filename,'.'),1));
